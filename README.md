...
1. 最近10场状态：胜率、进失球、对手强度和主客场拆分。
2. 伤病与停赛：按位置、预计出场时间和球队依赖度评分。
3. 预计与确认首发：官方首发公布后重新计算攻防强度。
4. 赛程疲劳：休息天数、旅行距离、加时赛和连续作战。
5. 比赛动机：出线形势、必须取胜和轮换风险。
6. 战术克制：阵型、控球、高位逼抢和防守反击匹配。
7. 天气与场地：温度、湿度、海拔、草皮和东道主环境。
8. 临场赔率：仅作为综合校准，不替代球队基本面。

权重为近期状态 20%、伤停 20%、首发 15%、疲劳 10%、动机 10%、
战术 10%、天气场地 5%、市场 10%。可信特征先调整双方预期进球，再进入
Poisson 比分矩阵和胜平负模型。

情报遵循严格来源规则：

- `confirmed`、`verified`、`automatic` 才会改变预测。
- `missing`、`pending_official`、媒体猜测和模型代理保持中性。
- 官方首发或伤停标记为 `confirmed` 时必须提供来源 URL，否则数据生成失败。
- 情报缺失会降低完整度和信心等级，但系统不会自行编造内容。

人工复核入口是 `data/official_intelligence.json`。每项可保存官方机构、来源网址、
发布时间、摘要、球员明细以及主客队影响值。比赛详情页会展示八项状态和来源链接。
